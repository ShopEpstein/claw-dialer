...
return `<?xml version="1.0" encoding="UTF-8"?> <Response> <Gather input="speech" action="${url}" method="POST" speechTimeout="2" speechModel="phone_call" enhanced="true" timeout="8"> <Say voice="Polly.Matthew">${escapeXml(sayText)}</Say> </Gather> <Say voice="Polly.Matthew">I didn't catch that. No problem, have a great day.</Say> <Hangup/> </Response>`;
}
...
if (action === ‘twiml’) {
res.setHeader(‘Content-Type’, ‘text/xml’);
return res.status(200).send(`<?xml version="1.0" encoding="UTF-8"?> <Response> <Play>${PITCH_URL}</Play> <Gather numDigits="1" action="${BASE}/api/twilio?action=gather" method="POST" timeout="8"> <Play>${PITCH_URL}</Play> </Gather> <Say voice="Polly.Matthew">No problem. Have a great day.</Say> <Hangup/> </Response>`);
}
...
res.setHeader(‘Content-Type’, ‘text/xml’);
return res.status(200).send(`<?xml version="1.0" encoding="UTF-8"?> <Response> <Say voice="Polly.Matthew">${digit === '1' ? 'Perfect. Check your texts in just a moment. Talk soon.' : 'No problem. Have a great day.'}</Say> <Hangup/> </Response>`);
}
