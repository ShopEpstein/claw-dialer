...
const SMS_FOLLOW_UP = (name) =>
  `Hi ${name}, I just tried reaching you about a tool helping dealerships get more leads and drop software costs. Happy to walk you through it — just reply here.`;
...
export default function ClawDialer() {
  ...
  function toggleAgent() {
    ...
    if (next) {
      ...
      notify('AI Agent Mode ACTIVE', 'info');
  ...
  async function sendSMS() {
    ...
    try {
      ...
      const data = await r.json();
      if (!r.ok) throw new Error(data.error);
      notify(`SMS sent ✓ SID: ${data.messageSid}`, 'success');
      setSmsModal(false);
    } catch (err) {
      notify(`SMS failed: ${err.message}`, 'warning');
    }
  }
    ...
    <>
      ...
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'0 20px',height:50,background:'var(--surface)',borderBottom:'1px solid var(--border)',position:'sticky',top:0,zIndex:100}}>
        <div style={{display:'flex',alignItems:'center',gap:16}}>
          <span style={{fontFamily:'Bebas Neue, sans-serif',fontSize:20,letterSpacing:3,color:'var(--teal)',textShadow:'0 0 20px rgba(20,241,198,0.4)'}}>CLAW DIALER <span style={{color:'var(--text-dim)',fontSize:10,fontFamily:'DM Mono, monospace',letterSpacing:2,verticalAlign:'middle'}}>// COMMAND CENTER</span></span>
          <span style={{display:'flex',alignItems:'center',gap:6,padding:'3px 10px',borderRadius:2,fontFamily:'DM Mono, monospace',fontSize:10,letterSpacing:1,background:'rgba(46,255,154,0.08)',border:'1px solid rgba(46,255,154,0.2)',color:'var(--green)'}}>
            <span style={{width:6,height:6,borderRadius:'50%',background:'var(--green)',animation:'pulse 2s infinite',display:'inline-block'}}></span>
            TWILIO READY
          </span>
        </div>
      ...
      {tab === 'dialer' && (
        <div style={{display:'grid',gridTemplateColumns:'320px 1fr 280px',height:'calc(100vh - 90px)',overflow:'hidden'}}>
          ...
          <div style={{borderRight:'1px solid var(--border)',overflow:'hidden',display:'flex',flexDirection:'column'}}>
            ...
            <div style={{flex:1,overflowY:'auto'}}>
              {filteredContacts.length === 0 ? (
                ...
                const sColor = { new:'#3B8FFF', called:'#FFD600', interested:'#14F1C6', voicemail:'#6B7A8D', callback:'#FF6B2B', 'not-interested':'#FF3B3B' }[c.status||'new'];
                return (
                  <div key={c.id} onClick={() => selectContact(idx)} style={{padding:'10px 14px',borderBottom:'1px solid var(--border)',cursor:'pointer',display:'flex',alignItems:'center',gap:10,background: activeIdx===idx ? 'var(--surface3)' : 'transparent',borderLeft: activeIdx===idx ? '2px solid var(--teal)' : '2px solid transparent',transition:'background 0.15s'}}>
                    ...
                    <div style={{fontFamily:'DM Mono, monospace',fontSize:8,padding:'2px 6px',borderRadius:2,color:sColor,border:`1px solid ${sColor}33`,background:`${sColor}11`,flexShrink:0,letterSpacing:1}}>{(c.status||'NEW').toUpperCase()}</div>
                  </div>
                );
              })}
            </div>
          ...
          <div style={{borderRight:'1px solid var(--border)',display:'flex',flexDirection:'column',overflow:'hidden'}}>
            ...
            <div style={{padding:'12px 20px',borderBottom:'1px solid var(--border)',background:'var(--surface)',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
              ...
              <div style={{display:'flex',alignItems:'center',gap:10}}>
                ...
                <div onClick={toggleAgent} style={{width:38,height:19,background: agentMode ? 'rgba(255,107,43,0.3)' : 'var(--surface3)',border:`1px solid ${agentMode ? 'var(--orange)' : 'var(--border2)'}`,borderRadius:10,cursor:'pointer',position:'relative',transition:'all 0.3s'}}>
                  <div style={{position:'absolute',width:13,height:13,borderRadius:'50%',background: agentMode ? 'var(--orange)' : 'var(--text-dim)',top:2,left: agentMode ? 21 : 2,transition:'all 0.3s',boxShadow: agentMode ? '0 0 8px rgba(255,107,43,0.6)' : 'none'}}></div>
                </div>
                {agentMode && <button onClick={() => setAgentPaused(p => !p)} style={{padding:'4px 10px',fontFamily:'Barlow Condensed, sans-serif',fontSize:10,fontWeight:700,letterSpacing:1.5,cursor:'pointer',border:'1px solid var(--border2)',background:'transparent',color:'var(--text-dim)',borderRadius:2}}>{agentPaused ? 'RESUME' : 'PAUSE'}</button>}
              </div>
            </div>
            ...
            {/* Active Call */}
            <div style={{padding:'18px 20px',borderBottom:'1px solid var(--border)',background:'var(--surface)',animation: callState==='connected' ? 'none' : 'none'}}>
              <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:14}}>
                <div>
                  ...
                  <div style={{fontFamily:'DM Mono, monospace',fontSize:11,color:'var(--text-dim)',marginTop:4}}>{activeContact?.business_name || 'Choose from the list to begin'}</div>
                  <div style={{fontFamily:'DM Mono, monospace',fontSize:13,color:'var(--teal)',marginTop:6}}>{activeContact?.phone || '—'}</div>
                </div>
                {callState !== 'idle' && (
                  <div style={{fontFamily:'DM Mono, monospace',fontSize:30,color:'var(--teal)',letterSpacing:4,textShadow:'0 0 20px rgba(20,241,198,0.4)'}}>{fmtTime(callSeconds)}</div>
                )}
              </div>
              ...
              <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
                {callState === 'idle' && (
                  <button onClick={startCall} style={{padding:'12px 24px',fontFamily:'Bebas Neue, sans-serif',fontSize:15,letterSpacing:3,background:'var(--green)',color:'var(--bg)',border:'none',cursor:'pointer',borderRadius:2,transition:'all 0.15s'}}>📞 DIAL</button>
                )}
                {['dialing','connected'].includes(callState) && (
                  <>
                    ...
                    <button onClick={() => setDisposition('voicemail')} style={{padding:'12px 16px',fontFamily:'Barlow Condensed, sans-serif',fontSize:11,fontWeight:700,letterSpacing:1,background:'transparent',color:'var(--text-dim)',border:'1px solid var(--border2)',cursor:'pointer',borderRadius:2}}>🎙 DROP VM</button>
                  </>
                )}
                <button onClick={() => activeContact ? setSmsModal(true) : notify('Select a contact','warning')} style={{padding:'12px 16px',fontFamily:'Barlow Condensed, sans-serif',fontSize:11,fontWeight:700,letterSpacing:1,background:'transparent',color:'var(--text-dim)',border:'1px solid var(--border2)',cursor:'pointer',borderRadius:2}}>💬 SMS</button>
              </div>
              ...
              {['connected','ended'].includes(callState) && (
                <div style={{display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:6,marginTop:14,paddingTop:14,borderTop:'1px solid var(--border)'}}>
                  {[['answered','✅ ANSWERED','var(--green)'],['voicemail','📬 VOICEMAIL','var(--text-dim)'],['callback','🔁 CALLBACK','var(--orange)'],['interested','🔥 INTERESTED','var(--teal)'],['not-interested','❌ NOT INT.','var(--red)']].map(([outcome,label,color]) => (
                    <button key={outcome} onClick={() => setDisposition(outcome)} style={{padding:'9px 4px',fontFamily:'Barlow Condensed, sans-serif',fontSize:10,fontWeight:700,letterSpacing:0.8,textTransform:'uppercase',cursor:'pointer',border:`1px solid ${color}44`,background:`${color}11`,color,borderRadius:2,transition:'all 0.15s',textAlign:'center'}}>
                      {label}
                    </button>
                  ))}
                </div>
            ...
            <div style={{padding:'12px 20px',borderBottom:'1px solid var(--border)'}}>
              <div style={{fontFamily:'DM Mono, monospace',fontSize:9,color:'var(--text-dim)',letterSpacing:2,textTransform:'uppercase',marginBottom:6}}>// CALL NOTES</div>
            ...
            <div style={{flex:1,overflowY:'auto',padding:'12px 20px'}}>
              <div style={{display:'flex',gap:6,marginBottom:12,flexWrap:'wrap'}}>
                {scripts.map((s,i) => (
                  <button key={i} onClick={() => setScriptIdx(i)} style={{padding:'5px 12px',fontSize:9,fontFamily:'Barlow Condensed, sans-serif',fontWeight:700,letterSpacing:1.5,cursor:'pointer',border:`1px solid ${scriptIdx===i ? s.color+'88' : 'var(--border2)'}`,background: scriptIdx===i ? 'var(--surface3)' : 'transparent',color: scriptIdx===i ? s.color : 'var(--text-dim)',borderRadius:2,textTransform:'uppercase'}}>
                    {s.name}
                  </button>
                ))}
              </div>
          ...
          {/* RIGHT: STATS + LOG */}
          <div style={{background:'var(--surface)',overflowY:'auto'}}>
            ...
            {[['Total Calls',totalCalls,'var(--teal)'],['Answer Rate',`${answerRate}%`,'var(--green)'],['Interested',`${intRate}%`,'var(--orange)']].map(([label,val,color]) => (
            ...
            <div style={{padding:'14px',borderBottom:'1px solid var(--border)',background:'rgba(20,241,198,0.03)',borderLeft:'2px solid var(--teal)'}}>
              <div style={{fontFamily:'DM Mono, monospace',fontSize:9,color:'var(--text-dim)',letterSpacing:2,textTransform:'uppercase',marginBottom:5}}>EST. PIPELINE MRR</div>
              <div style={{fontFamily:'Bebas Neue, sans-serif',fontSize:26,color:'var(--teal)',textShadow:'0 0 12px rgba(20,241,198,0.3)',letterSpacing:2}}>${pipeline.toLocaleString()}</div>
            ...
            {callLog.length === 0 ? (
              ...
              return (
                <div key={entry.id} style={{padding:'9px 14px',borderBottom:'1px solid var(--border)',display:'flex',alignItems:'center',gap:8}}>
                  <div style={{width:6,height:6,borderRadius:'50%',background:c,flexShrink:0,boxShadow: entry.outcome==='interested' ? '0 0 6px rgba(20,241,198,0.5)' : 'none'}}></div>
                  <div style={{flex:1,minWidth:0}}>
        ...
        <div style={{padding:24,overflowY:'auto',height:'calc(100vh - 90px)'}}>
          ...
          <div style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:2,overflow:'hidden'}}>
            ...
            <table style={{width:'100%',borderCollapse:'collapse'}}>
              ...
              <tbody>
                {[['INTERESTED 🔥',callLog.filter(c=>c.outcome==='interested').length,'var(--teal)'],['ANSWERED',callLog.filter(c=>c.outcome==='answered').length,'var(--green)'],['CALLBACK',callLog.filter(c=>c.outcome==='callback').length,'var(--orange)'],['VOICEMAIL',callLog.filter(c=>c.outcome==='voicemail').length,'var(--text-dim)'],['NOT INTERESTED',callLog.filter(c=>c.outcome==='not-interested').length,'var(--red)']].map(([label,count,color]) => (
      ...
      {tab === 'admin' && (
        <div style={{padding:24,overflowY:'auto',height:'calc(100vh - 90px)'}}>
          ...
          <div style={{marginBottom:28}}>
            ...
            <label style={{display:'block',border:'1px dashed var(--border2)',padding:28,textAlign:'center',cursor:'pointer',background:'var(--surface2)',borderRadius:2,transition:'all 0.2s'}}>
          ...
          <div style={{marginBottom:28}}>
                ...
                <textarea
                  ...
                  onBlur={e => {
                    ...
                    if (sections.length > 0) {
                      setScripts(prev => { const next=[...prev]; next[i]={...next[i],sections}; return next; });
                    }
                  }}
            ...
            <button onClick={() => notify('Scripts auto-saved on edit','success')} style={{padding:'9px 18px',fontFamily:'Barlow Condensed, sans-serif',fontSize:11,fontWeight:700,letterSpacing:1.5,background:'var(--teal)',color:'var(--bg)',border:'none',cursor:'pointer',borderRadius:2}}>SAVE SCRIPTS</button>
          </div>
          ...
          <div>
            ...
            <div style={{display:'flex',gap:10,flexWrap:'wrap'}}>
              ...
              <button onClick={() => { if(confirm('Clear everything?')) { setContacts([]); setCallLog([]); notify('Cleared','warning'); } }} style={{padding:'9px 16px',fontFamily:'Barlow Condensed, sans-serif',fontSize:11,fontWeight:700,letterSpacing:1,background:'var(--red)',color:'white',border:'none',cursor:'pointer',borderRadius:2,marginLeft:'auto'}}>CLEAR ALL</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.8)',zIndex:200,display:'flex',alignItems:'center',justifyContent:'center'}}>
          <div style={{background:'var(--surface)',border:'1px solid var(--border2)',borderRadius:2,padding:24,width:440,animation:'slideUp 0.2s ease'}}>
            ...
            <div style={{marginBottom:12}}>
              <div style={{fontFamily:'DM Mono, monospace',fontSize:9,color:'var(--text-dim)',letterSpacing:1,textTransform:'uppercase',marginBottom:6}}>TO</div>
            ...
            <div style={{marginBottom:6}}>
              <div style={{fontFamily:'DM Mono, monospace',fontSize:9,color:'var(--text-dim)',letterSpacing:1,textTransform:'uppercase',marginBottom:6}}>MESSAGE</div>
            ...
            <div style={{display:'flex',gap:8,justifyContent:'flex-end',marginTop:16}}>
              <button onClick={() => setSmsModal(false)} style={{padding:'9px 16px',fontFamily:'Barlow Condensed, sans-serif',fontSize:11,fontWeight:700,letterSpacing:1,background:'transparent',color:'var(--text-dim)',border:'1px solid var(--border2)',cursor:'pointer',borderRadius:2}}>CANCEL</button>
              <button onClick={sendSMS} style={{padding:'9px 16px',fontFamily:'Barlow Condensed, sans-serif',fontSize:11,fontWeight:700,letterSpacing:1,background:'var(--teal)',color:'var(--bg)',border:'none',cursor:'pointer',borderRadius:2}}>SEND SMS</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showAddModal && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.8)',zIndex:200,display:'flex',alignItems:'center',justifyContent:'center'}}>
          <div style={{background:'var(--surface)',border:'1px solid var(--border2)',borderRadius:2,padding:24,width:420,animation:'slideUp 0.2s ease'}}>
            ...
            <div style={{display:'flex',gap:8,justifyContent:'flex-end',marginTop:16}}>
              <button onClick={() => setShowAddModal(false)} style={{padding:'9px 16px',fontFamily:'Barlow Condensed, sans-serif',fontSize:11,fontWeight:700,letterSpacing:1,background:'transparent',color:'var(--text-dim)',border:'1px solid var(--border2)',cursor:'pointer',borderRadius:2}}>CANCEL</button>
              <button onClick={() => {
              ...
              }} style={{padding:'9px 16px',fontFamily:'Barlow Condensed, sans-serif',fontSize:11,fontWeight:700,letterSpacing:1,background:'var(--teal)',color:'var(--bg)',border:'none',cursor:'pointer',borderRadius:2}}>ADD</button>
            </div>
          </div>
        </div>
      )}
