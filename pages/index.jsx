...
const DEFAULT_SCRIPTS = [
  ...
  },
  {
    ...
    sections: [
      ...
      { label: 'COMPARISON', text: "An agency would charge you $5,000 setup and $1,500 a month for this. We're at $500 setup and $99 a month during our launch window. Founding customers keep $99 forever." },
...
const SKINS = {
  DEFAULT:    { accent: '#14F1C6', dim: '#0A9478', icon: '⚡', label: 'DEFAULT' },
  CYBERCLAW:  { accent: '#BF00FF', dim: '#7A00AA', icon: '💜', label: 'CYBERCLAW' },
  GOTHICCLAW: { accent: '#CC0000', dim: '#880000', icon: '🩸', label: 'GOTHICCLAW' },
  TACTICLAW:  { accent: '#7FFF00', dim: '#4ABF00', icon: '🎯', label: 'TACTICLAW' },
  ECONOSKIN:  { accent: '#FF6B2B', dim: '#CC4400', icon: '🔥', label: 'ECONOSKIN' },
  BUDGETSKIN: { accent: '#39FF14', dim: '#20CC00', icon: '📊', label: 'BUDGETSKIN' },
};

const SMS_TEMPLATES = {
  VINHUNTER:     (n) => `Chase @ VinHunter: Hey ${n}, just tried reaching you. Free lot audit shows what buyers find when they Google your VINs — https://vinledgerai.live/pricing Founding rate locks forever. Reply STOP to opt out.`,
  ECONOCLAW:     (n) => `Chase @ EconoClaw: Hey ${n}, 21 AI agents working your business 24/7 for $99/mo. Launch pricing locks forever for founding customers. https://econoclaw.vercel.app Reply STOP to opt out.`,
  WHITEGLOVECLAW:(n) => `Chase @ WhiteGloveClaw: Hey ${n}, full AI infrastructure at 20% below market. VPS $2,400 · Mac Mini $4,000 · In-person $4,800. Call (850) 341-4324. Reply STOP to opt out.`,
  RENTACLAW:     (n) => `Chase @ RentAClaw: Hey ${n}, try 21 AI agents for $49/week — personal refund if no value. https://econoclaw.vercel.app Reply STOP to opt out.`,
  BUDGETCLAW:    (n) => `Chase @ BudgetClaw: Hey ${n}, your current stack costs ~$6,188/yr. Ours: $2,687. 21 agents, replaces Zapier + HubSpot + VA. https://econoclaw.vercel.app Reply STOP to opt out.`,
  TRANSBID:      (n) => `Chase @ TransBid: Hey ${n}, 0.5% only when you win — zero upfront. Veterans 0% forever. https://transbid.live Reply STOP to opt out.`,
  CLAWAWAY:      (n) => `Chase @ ClawAway: Hey ${n}, build what you want, pay how you want — card, crypto, rev share, barter. (850) 341-4324. Reply STOP to opt out.`,
};

function getSMS(scriptName, contactName) {
...
function fmtTime(s) { const sec = s||0; return `${Math.floor(sec/60).toString().padStart(2,'0')}:${(sec%60).toString().padStart(2,'0')}` }
function initials(n) { return (n||'?').split(' ').map(w=>w[0]).join('').slice(0,2).toUpperCase() }
function storageGet(k, d) { try { return JSON.parse(localStorage.getItem(k)) ?? d } catch { return d } }
function storageSet(k, v) { try { localStorage.setItem(k, JSON.stringify(v)) } catch {} }

const STATUS_COLOR = { new:'#3B8FFF', called:'#FFD600', interested:'var(--accent)', voicemail:'#6B7A8D', callback:'#FF6B2B', 'not-interested':'#FF3B3B', dnc:'#FF3B3B' };
const CALL_COLOR   = { idle:'#6B7A8D', dialing:'#FFD600', connected:'#2EFF9A', ended:'#FF6B2B' };
const CALL_TEXT    = { idle:'STANDBY', dialing:'DIALING...', connected:'CONNECTED', ended:'CALL ENDED' };
...
function LoginScreen({ onLogin }) {
  ...
  return (
    <div style={{height:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:'#080A0F'}}>
      <div style={{textAlign:'center',transform:shake?'translateX(-8px)':'none',transition:'transform 0.1s'}}>
        ...
        <div style={{background:'#0D1017',border:'1px solid #1E2D40',borderRadius:4,padding:32,width:340,margin:'0 auto'}}>
          ...
          <input type="password" value={pw} onChange={e=>{setPw(e.target.value);setErr(false)}} onKeyDown={e=>e.key==='Enter'&&attempt()} placeholder="••••••••" autoFocus
          ...
          {err && <div style={{fontFamily:'DM Mono,monospace',fontSize:10,color:'#FF3B3B',marginBottom:12}}>ACCESS DENIED</div>}
          <button onClick={attempt} style={{width:'100%',padding:13,background:'#14F1C6',color:'#080A0F',fontFamily:'Bebas Neue,sans-serif',fontSize:16,letterSpacing:4,border:'none',cursor:'pointer',borderRadius:2}}>AUTHENTICATE</button>
        </div>
...
export default function ClawDialer() {
  ...
  const [mobilePanel,  setMobilePanel]  = useState('leads');
  ...
  useEffect(() => {
    const t = setInterval(() => {
      ...
      setTcpaOk(now.getHours()>=8 && now.getHours()<21);
    }, 1000);
    return () => clearInterval(t);
  }, []);
  useEffect(() => {
    const poll = setInterval(async () => { try { const r=await fetch('/api/agent?action=status'); setAgentStatus(await r.json()); } catch {} }, 5000);
  ...
  function selectContact(idx) {
    ...
    setSmsBody(getSMS(scripts[scriptIdx]?.name, contacts[idx]?.name));
    if (isMobile) setMobilePanel('dial');
  }

  function updateStatus(idx, status) {
    setContacts(prev => { const n=[...prev]; n[idx]={...n[idx],status,notes}; return n; });
  }

  function markDNC(idx) {
    setContacts(prev => { const n=[...prev]; n[idx]={...n[idx],dnc:true,status:'dnc'}; return n; });
    notify('Contact marked DNC','warning'); setActiveIdx(null);
  }
  ...
  function handleCSV(e) {
    ...
    reader.onload = ev => {
      ...
      if (lines.length<2) return notify('CSV empty','warning');
      const h = lines[0].split(',').map(x=>x.trim().toLowerCase().replace(/['"]/g,''));
      const ni = h.findIndex(x=>x.includes('name')&&!x.includes('business')&&!x.includes('company'));
      const bi = h.findIndex(x=>x.includes('business')||x.includes('company')||x.includes('biz'));
      const pi = h.findIndex(x=>x.includes('phone')||x.includes('mobile')||x.includes('cell'));
      const ei = h.findIndex(x=>x.includes('email'));
      const rows = [];
      for (let i=1;i<lines.length;i++) {
        const c = lines[i].split(',').map(x=>x.trim().replace(/^["']|["']$/g,''));
        const phone=pi>=0?c[pi]:'', email=ei>=0?c[ei]:'';
        if (!phone&&!email) continue;
        rows.push({id:Date.now()+i,name:ni>=0?c[ni]:'',business_name:bi>=0?c[bi]:'',phone,email,status:'new',notes:'',list_name:file.name.replace('.csv',''),created_at:new Date().toISOString()});
      }
      setContacts(prev=>[...prev,...rows]);
      notify(`Imported ${rows.length} contacts`,'success');
    };
  ...
  async function loadVinLedger() {
    setVinLoading(true);
    try {
      const r = await fetch(`https://vinledgerai.live/api/dealers/export?has_phone=true&limit=500&state=${vinState}`);
      const data = await r.json();
      const dealers = (data.dealers||data||[]).filter(d=>d.phone);
      const mapped = dealers.map((d,i)=>({id:Date.now()+i,name:d.name||d.contact_name||'',business_name:d.business_name||d.name||'',phone:d.phone,email:d.email||'',status:'new',notes:'',list_name:`VinLedger-${vinState}`,created_at:new Date().toISOString()}));
      setContacts(prev=>[...prev,...mapped]);
      notify(`Loaded ${mapped.length} dealers from VinLedger (${vinState})`,'success');
    } catch(err) { notify(`VinLedger failed: ${err.message}`,'warning'); }
    setVinLoading(false);
  }
  ...
  async function startCall() {
    ...
    setCallState('dialing'); setCallSeconds(0);
    clearInterval(timerRef.current);
    timerRef.current = setInterval(()=>setCallSeconds(s=>s+1), 1000);
    try {
      const r = await fetch('/api/twilio?action=call', {
        method:'POST', headers:{'Content-Type':'application/json'},
        body: JSON.stringify({to:activeContact.phone,contactId:activeContact.id,contactName:activeContact.name,contactEmail:activeContact.email||'',script:scripts[scriptIdx]?.name,aiMode})
      });
      const data = await r.json();
      if (!r.ok) throw new Error(data.error);
      setCallSid(data.callSid); setCallState('connected');
      notify(`Dialing ${activeContact.name||activeContact.phone}...`,'info');
    } catch(err) { setCallState('idle'); clearInterval(timerRef.current); notify(`Call failed: ${err.message}`,'warning'); }
  }

  async function endCall() {
    clearInterval(timerRef.current); setCallState('ended');
    if (callSid) {
      try { await fetch('/api/twilio?action=hangup',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({callSid})}); } catch {}
    }
  }

  async function sendEmail(contact, product) {
    if (!contact?.email) return;
    try {
      await fetch('/api/recordings?action=email', {
        method:'POST', headers:{'Content-Type':'application/json'},
        body: JSON.stringify({to:contact.email,contactName:contact.name,business:contact.business_name,product})
      });
    } catch {}
  }
  ...
  async function setDisposition(outcome) {
    ...
    clearInterval(timerRef.current); setCallState('idle');
    const entry = {id:Date.now(),contact_id:activeContact.id,name:activeContact.name,business:activeContact.business_name,phone:activeContact.phone,email:activeContact.email,outcome,duration:callSeconds,notes,script:scripts[scriptIdx]?.name,timestamp:new Date().toISOString()};
    setCallLog(prev=>[entry,...prev]);
    const sm = {answered:'called',voicemail:'voicemail',callback:'callback',interested:'interested','not-interested':'not-interested'};
    updateStatus(activeIdx, sm[outcome]||'called');
    if (outcome==='interested') {
      notify(`🔥 HOT LEAD! Auto-SMS + email firing for ${activeContact.name}`,'success');
      try { await fetch('/api/twilio?action=sms',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({to:activeContact.phone,body:getSMS(scripts[scriptIdx]?.name,activeContact.name)})}); } catch {}
      await sendEmail(activeContact, scripts[scriptIdx]?.name);
    }
  ...
  function toggleAgent() {
    ...
    if (next) { setAgentPaused(false); notify('Browser Agent ACTIVE','info'); setTimeout(agentNext, 1000); }
  ...
  function agentNext() {
    ...
    if (!newOnes.length) { setAgentMode(false); notify('Agent done — no new contacts','success'); return; }
    const idx = contacts.indexOf(newOnes[0]);
    selectContact(idx);
    setTimeout(()=>{ if(agentModeRef.current&&!agentPausedRef.current) startCall(); }, 1500);
  }

  async function serverAgent(action, body={}) {
    try {
      const r = await fetch(`/api/agent?action=${action}`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});
      const d = await r.json(); notify(d.message||`Agent ${action}`,'info');
  ...
  if (!authed) return <LoginScreen onLogin={()=>{setAuthed(true);storageSet('claw_authed',true);}}/>;
  ...
  const css = `
    ...
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    :root{
      ...
      --accent:${SK.accent};--accent-dim:${SK.dim};
      --teal:#14F1C6;--orange:#FF6B2B;--red:#FF3B3B;--green:#2EFF9A;--yellow:#FFD600;--blue:#3B8FFF;
  ...
  return (
    <>
      ...
      {/* TOP BAR */}
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'0 16px',height:46,background:'var(--surface)',borderBottom:'1px solid var(--border)',flexShrink:0}}>
        <div style={{display:'flex',alignItems:'center',gap:12}}>
          <span style={{fontFamily:'Bebas Neue,sans-serif',fontSize:18,letterSpacing:3,color:'var(--accent)',textShadow:`0 0 20px ${SK.accent}66`}}>{SK.icon} CLAW DIALER</span>
          <span style={{display:'flex',alignItems:'center',gap:5,padding:'2px 8px',borderRadius:2,fontFamily:'DM Mono,monospace',fontSize:9,background:'rgba(46,255,154,0.08)',border:'1px solid rgba(46,255,154,0.2)',color:'var(--green)'}}>
          ...
          {!tcpaOk && <span style={{padding:'2px 8px',borderRadius:2,fontFamily:'DM Mono,monospace',fontSize:9,background:'rgba(255,59,59,0.1)',border:'1px solid rgba(255,59,59,0.3)',color:'var(--red)'}}>⚠ TCPA BLOCKED</span>}
        </div>
        <div style={{display:'flex',gap:12,fontFamily:'DM Mono,monospace',fontSize:10,color:'var(--text-dim)',alignItems:'center'}}>
          {!isMobile && <><span>+1 (855) 960-0110</span><span style={{color:'var(--border2)'}}>|</span></>}
          <span style={{color:'var(--accent)'}}>{clock}</span>
          <button onClick={()=>{setAuthed(false);storageSet('claw_authed',false);}} style={{padding:'2px 8px',fontSize:8,fontFamily:'Barlow Condensed,sans-serif',fontWeight:700,cursor:'pointer',border:'1px solid var(--border2)',background:'transparent',color:'var(--text-dim)',borderRadius:2}}>LOCK</button>
        </div>
      </div>
      ...
      <div style={{display:'flex',background:'var(--surface)',borderBottom:'1px solid var(--border)',padding:'0 16px',flexShrink:0}}>
        {['dialer','dashboard','admin'].map(t=>(
          <button key={t} onClick={()=>setTab(t)} style={{padding:'8px 16px',fontFamily:'Barlow Condensed,sans-serif',fontSize:10,fontWeight:700,letterSpacing:2,textTransform:'uppercase',color:tab===t?'var(--accent)':'var(--text-dim)',cursor:'pointer',border:'none',borderBottom:tab===t?`2px solid var(--accent)`:'2px solid transparent',background:'none'}}>
            {t.toUpperCase()}
          </button>
        ))}
        {tab==='dialer' && isMobile && (
          <>{'\u00A0'.repeat(1)}<div style={{flex:1}}/>
            {['leads','dial','log'].map(p=>(
              <button key={p} onClick={()=>setMobilePanel(p)} style={{padding:'8px 12px',fontFamily:'Barlow Condensed,sans-serif',fontSize:10,fontWeight:700,letterSpacing:1,color:mobilePanel===p?'var(--accent)':'var(--text-dim)',cursor:'pointer',border:'none',borderBottom:mobilePanel===p?`2px solid var(--accent)`:'2px solid transparent',background:'none'}}>
                {p.toUpperCase()}
              </button>
            ))}
          </>
      ...
      {/* ── DIALER TAB ── */}
      {tab==='dialer' && (
        <div style={{display: isMobile?'block':'grid', gridTemplateColumns:'300px 1fr 260px', height:'calc(100vh - 90px)', overflow:'hidden'}}>

          {/* CONTACTS PANEL */}
          {(!isMobile || mobilePanel==='leads') && (
            <div style={{borderRight:'1px solid var(--border)',overflow:'hidden',display:'flex',flexDirection:'column',height:'100%'}}>
              <div style={{padding:'10px 14px',borderBottom:'1px solid var(--border)',display:'flex',alignItems:'center',justifyContent:'space-between',background:'var(--surface)'}}>
                ...
                <div style={{display:'flex',gap:6,alignItems:'center'}}>
                  <span style={{fontFamily:'DM Mono,monospace',fontSize:10,color:'var(--text-dim)'}}>{filtered.length}/{contacts.length}</span>
                  ...
                  <button onClick={()=>{setSelectMode(s=>!s);setSelectedIds(new Set());}} style={{padding:'3px 8px',fontSize:8,fontFamily:'Barlow Condensed,sans-serif',fontWeight:700,cursor:'pointer',border:`1px solid ${selectMode?'var(--accent)':'var(--border2)'}`,background:selectMode?'var(--surface3)':'transparent',color:selectMode?'var(--accent)':'var(--text-dim)',borderRadius:2}}>
              ...
              <div style={{padding:'8px 10px',borderBottom:'1px solid var(--border)',display:'flex',gap:6}}>
                ...
                <button onClick={()=>setAddModal(true)} style={{padding:'6px 10px',background:'var(--accent)',color:'var(--bg)',fontFamily:'Barlow Condensed,sans-serif',fontSize:11,fontWeight:700,border:'none',cursor:'pointer',borderRadius:2,flexShrink:0}}>+ ADD</button>
              </div>
              <div style={{padding:'5px 10px',borderBottom:'1px solid var(--border)',display:'flex',gap:4,flexWrap:'wrap'}}>
                {['all','new','callback','interested','voicemail'].map(f=>(
                  <button key={f} onClick={()=>setStatusFilter(f)} style={{padding:'3px 8px',fontSize:8,fontFamily:'Barlow Condensed,sans-serif',fontWeight:700,letterSpacing:1,cursor:'pointer',border:`1px solid ${statusFilter===f?'var(--accent-dim)':'var(--border2)'}`,background:statusFilter===f?'var(--surface3)':'transparent',color:statusFilter===f?'var(--accent)':'var(--text-dim)',borderRadius:2,textTransform:'uppercase'}}>
              ...
              <div style={{flex:1,overflowY:'auto'}}>
                {filtered.length===0 ? (
                  ...
                  const sc = STATUS_COLOR[c.status||'new']||'#3B8FFF';
                  return (
                    <div key={c.id} onClick={()=>selectMode?setSelectedIds(prev=>{const n=new Set(prev);n.has(c.id)?n.delete(c.id):n.add(c.id);return n;}):selectContact(idx)}
                      ...
                      <div style={{fontFamily:'DM Mono,monospace',fontSize:7,padding:'2px 5px',borderRadius:2,color:sc,border:`1px solid ${sc}33`,background:`${sc}11`,flexShrink:0,letterSpacing:1}}>{(c.status||'NEW').toUpperCase()}</div>
                    </div>
                  );
                })}
              </div>
            ...
            <div style={{borderRight:'1px solid var(--border)',display:'flex',flexDirection:'column',overflow:'hidden',height:'100%'}}>
              ...
              <div style={{padding:'8px 16px',borderBottom:'1px solid var(--border)',background:'var(--surface)',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                ...
                <div style={{display:'flex',alignItems:'center',gap:8}}>
                  ...
                  <div onClick={toggleAgent} style={{width:34,height:17,background:agentMode?'rgba(255,107,43,0.3)':'var(--surface3)',border:`1px solid ${agentMode?'var(--orange)':'var(--border2)'}`,borderRadius:10,cursor:'pointer',position:'relative'}}>
                    <div style={{position:'absolute',width:11,height:11,borderRadius:'50%',background:agentMode?'var(--orange)':'var(--text-dim)',top:2,left:agentMode?19:2,transition:'left 0.3s'}}></div>
                  </div>
                  {agentMode && <button onClick={()=>setAgentPaused(p=>!p)} style={{padding:'3px 8px',fontFamily:'Barlow Condensed,sans-serif',fontSize:9,fontWeight:700,cursor:'pointer',border:'1px solid var(--border2)',background:'transparent',color:'var(--text-dim)',borderRadius:2}}>{agentPaused?'RESUME':'PAUSE'}</button>}
                  <div onClick={()=>setAiMode(m=>!m)} style={{display:'flex',alignItems:'center',gap:4,padding:'3px 8px',borderRadius:2,border:`1px solid ${aiMode?'var(--accent)':'var(--border2)'}`,background:aiMode?'rgba(20,241,198,0.08)':'transparent',cursor:'pointer'}}>
                    <span style={{fontFamily:'DM Mono,monospace',fontSize:8,color:aiMode?'var(--accent)':'var(--text-dim)',letterSpacing:1}}>AI {aiMode?'ON':'OFF'}</span>
                  </div>
                </div>
              </div>
              ...
              <div style={{padding:'8px 16px',borderBottom:'1px solid var(--border)',display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:6}}>
                {[['QUEUE',queuedCount,'var(--text)'],['CALLED',totalCalls,'var(--text)'],['HOT',interestedCalls,'var(--accent)'],['MRR',`$${pipeline}`,'var(--green)']].map(([l,v,c])=>(
                  <div key={l} style={{background:'var(--surface2)',border:'1px solid var(--border)',borderRadius:2,padding:'6px 8px',textAlign:'center'}}>
                    <div style={{fontFamily:'DM Mono,monospace',fontSize:16,color:c,lineHeight:1}}>{v}</div>
                    <div style={{fontSize:8,color:'var(--text-dim)',letterSpacing:1,marginTop:2}}>{l}</div>
                  </div>
                ))}
              </div>

              {/* Active contact + call controls */}
              <div style={{padding:'14px 16px',borderBottom:'1px solid var(--border)',background:'var(--surface)'}}>
                <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:10}}>
                  ...
                  {callState!=='idle' && <div style={{fontFamily:'DM Mono,monospace',fontSize:26,color:'var(--accent)',letterSpacing:4}}>{fmtTime(callSeconds)}</div>}
                </div>
                <div style={{display:'flex',alignItems:'center',gap:6,marginBottom:10,fontFamily:'DM Mono,monospace',fontSize:10}}>
                  <div style={{width:7,height:7,borderRadius:'50%',background:CALL_COLOR[callState],animation:['dialing','connected'].includes(callState)?'pulse 1s infinite':'none'}}></div>
                  <span style={{color:CALL_COLOR[callState]}}>{CALL_TEXT[callState]}</span>
                  {!tcpaOk && <span style={{color:'var(--red)',fontSize:9,marginLeft:8}}>⚠ TCPA BLOCK</span>}
                </div>
                <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>
                  ...
                  {['dialing','connected'].includes(callState) && (
                    <>
                      ...
                      <button onClick={()=>setDisposition('voicemail')} style={{padding:'10px 12px',fontFamily:'Barlow Condensed,sans-serif',fontSize:10,fontWeight:700,background:'transparent',color:'var(--text-dim)',border:'1px solid var(--border2)',cursor:'pointer',borderRadius:2}}>🎙 DROP VM</button>
                    </>
                  )}
                  <button onClick={()=>activeContact?setSmsModal(true):notify('Select a contact','warning')} style={{padding:'10px 12px',fontFamily:'Barlow Condensed,sans-serif',fontSize:10,fontWeight:700,background:'transparent',color:'var(--text-dim)',border:'1px solid var(--border2)',cursor:'pointer',borderRadius:2}}>💬 SMS</button>
                  {activeContact?.email && (
                    <button onClick={async()=>{await sendEmail(activeContact,scripts[scriptIdx]?.name);notify(`📧 Email sent to ${activeContact.email}`,'success');}} style={{padding:'10px 12px',fontFamily:'Barlow Condensed,sans-serif',fontSize:10,fontWeight:700,background:'transparent',color:'var(--teal)',border:'1px solid rgba(20,241,198,0.3)',cursor:'pointer',borderRadius:2}}>📧 EMAIL</button>
                  )}
                  {activeContact && <button onClick={()=>markDNC(activeIdx)} style={{padding:'10px 12px',fontFamily:'Barlow Condensed,sans-serif',fontSize:10,fontWeight:700,background:'transparent',color:'var(--red)',border:'1px solid rgba(255,59,59,0.3)',cursor:'pointer',borderRadius:2}}>DNC</button>}
                </div>
                {['connected','ended'].includes(callState) && (
                  <div style={{display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:5,marginTop:10,paddingTop:10,borderTop:'1px solid var(--border)'}}>
                    {[['answered','✅ ANS','var(--green)'],['voicemail','📬 VM','var(--text-dim)'],['callback','🔁 CB','var(--orange)'],['interested','🔥 HOT','var(--accent)'],['not-interested','❌ NO','var(--red)']].map(([outcome,label,color])=>(
                      <button key={outcome} onClick={()=>setDisposition(outcome)} style={{padding:'8px 4px',fontFamily:'Barlow Condensed,sans-serif',fontSize:9,fontWeight:700,cursor:'pointer',border:`1px solid ${color}44`,background:`${color}11`,color,borderRadius:2,textAlign:'center'}}>
                        {label}
                      </button>
                    ))}
                  </div>
          ...
          {(!isMobile || mobilePanel==='log') && (
            <div style={{background:'var(--surface)',overflowY:'auto',height:'100%'}}>
              ...
              {[['Total Calls',totalCalls,'var(--accent)'],['Answer Rate',`${answerRate}%`,'var(--green)'],['Interested',`${intRate}%`,'var(--orange)']].map(([l,v,c])=>(
                <div key={l} style={{padding:'12px',borderBottom:'1px solid var(--border)'}}>
                  <div style={{fontFamily:'DM Mono,monospace',fontSize:8,color:'var(--text-dim)',letterSpacing:2,textTransform:'uppercase',marginBottom:4}}>{l}</div>
                  <div style={{fontFamily:'Bebas Neue,sans-serif',fontSize:28,letterSpacing:2,color:c,lineHeight:1}}>{v}</div>
                </div>
              ))}
              <div style={{padding:'12px',borderBottom:'1px solid var(--border)',borderLeft:'2px solid var(--accent)'}}>
                <div style={{fontFamily:'DM Mono,monospace',fontSize:8,color:'var(--text-dim)',letterSpacing:2,textTransform:'uppercase',marginBottom:4}}>PIPELINE MRR</div>
              ...
              {agentStatus && (
                <div style={{padding:'10px 12px',borderBottom:'1px solid var(--border)',background:'var(--surface2)'}}>
                  ...
                  {agentStatus.status!=='idle' && <div style={{fontFamily:'DM Mono,monospace',fontSize:9,color:'var(--text-dim)',marginBottom:6}}>{agentStatus.called}/{agentStatus.total} · {agentStatus.queue} queued</div>}
                  <div style={{display:'flex',gap:4}}>
                    {agentStatus.status==='idle'    && <button onClick={()=>serverAgent('start',{state:vinState})} style={{padding:'4px 8px',fontSize:8,fontFamily:'Barlow Condensed,sans-serif',fontWeight:700,cursor:'pointer',border:'1px solid var(--green)',background:'transparent',color:'var(--green)',borderRadius:2}}>▶ START</button>}
                    {agentStatus.status==='running'  && <><button onClick={()=>serverAgent('pause')} style={{padding:'4px 8px',fontSize:8,fontFamily:'Barlow Condensed,sans-serif',fontWeight:700,cursor:'pointer',border:'1px solid var(--yellow)',background:'transparent',color:'var(--yellow)',borderRadius:2}}>⏸ PAUSE</button><button onClick={()=>serverAgent('stop')} style={{padding:'4px 8px',fontSize:8,fontFamily:'Barlow Condensed,sans-serif',fontWeight:700,cursor:'pointer',border:'1px solid var(--red)',background:'transparent',color:'var(--red)',borderRadius:2}}>■ STOP</button></>}
                    {agentStatus.status==='paused'   && <button onClick={()=>serverAgent('resume')} style={{padding:'4px 8px',fontSize:8,fontFamily:'Barlow Condensed,sans-serif',fontWeight:700,cursor:'pointer',border:'1px solid var(--green)',background:'transparent',color:'var(--green)',borderRadius:2}}>▶ RESUME</button>}
                  </div>
                </div>
              )}
                ...
                return (
                  <div key={entry.id} style={{padding:'8px 12px',borderBottom:'1px solid var(--border)',display:'flex',alignItems:'center',gap:7}}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      ...
                      <div style={{fontFamily:'DM Mono,monospace',fontSize:8,color:'var(--text-dim)',marginTop:1}}>{(entry.outcome||'').toUpperCase()} · {fmtTime(entry.duration)}</div>
                    </div>
                  </div>
                );
              })}
      ...
      {tab==='dashboard' && (
        <div style={{padding:20,overflowY:'auto',height:'calc(100vh - 90px)'}}>
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(180px,1fr))',gap:14,marginBottom:20}}>
            {[['Total Calls',totalCalls,'var(--accent)',totalCalls],['Answer Rate',`${answerRate}%`,'var(--green)',answerRate],['Hot Leads',interestedCalls,'var(--orange)',intRate],['Pipeline MRR',`$${pipeline}`,'var(--accent)',100]].map(([l,v,c,p])=>(
              <div key={l} style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:2,padding:18}}>
                <div style={{fontFamily:'Bebas Neue,sans-serif',fontSize:34,letterSpacing:2,color:c,lineHeight:1,marginBottom:4}}>{v}</div>
                <div style={{fontFamily:'DM Mono,monospace',fontSize:8,color:'var(--text-dim)',letterSpacing:2,textTransform:'uppercase'}}>{l}</div>
                <div style={{height:2,background:'var(--surface3)',borderRadius:2,marginTop:8,overflow:'hidden'}}>
                  <div style={{height:'100%',background:c,width:`${Math.min(p,100)}%`,transition:'width 0.5s'}}></div>
                </div>
              </div>
            ))}
          </div>
          <div style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:2,overflow:'hidden'}}>
            ...
            <table style={{width:'100%',borderCollapse:'collapse'}}>
              ...
              <tbody>
                {[['INTERESTED 🔥','interested','var(--accent)'],['ANSWERED','answered','var(--green)'],['CALLBACK','callback','var(--orange)'],['VOICEMAIL','voicemail','var(--text-dim)'],['NOT INTERESTED','not-interested','var(--red)']].map(([label,key,color])=>{
        ...
        <div style={{padding:20,overflowY:'auto',height:'calc(100vh - 90px)'}}>
          ...
          <div style={{marginBottom:24}}>
            ...
            <div style={{display:'flex',gap:8,alignItems:'center'}}>
              ...
              <button onClick={loadVinLedger} disabled={vinLoading} style={{padding:'9px 18px',fontFamily:'Barlow Condensed,sans-serif',fontSize:11,fontWeight:700,background:vinLoading?'var(--surface3)':'var(--accent)',color:vinLoading?'var(--text-dim)':'var(--bg)',border:'none',cursor:vinLoading?'wait':'pointer',borderRadius:2,display:'flex',alignItems:'center',gap:6}}>
                {vinLoading?<span style={{display:'inline-block',animation:'spin 0.8s linear infinite'}}>⟳</span>:'📡'} LOAD DEALERS
              </button>
            </div>
          </div>
              ...
              <div key={i} style={{marginBottom:14}}>
                ...
                <textarea defaultValue={s.sections.map(sec=>`[${sec.label}]\n${sec.text}`).join('\n\n')}
                  onBlur={e=>{
                    ...
                    if(sections.length>0) setScripts(prev=>{const n=[...prev];n[i]={...n[i],sections};return n;});
                  }}
      ...
      {addModal && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.85)',zIndex:200,display:'flex',alignItems:'center',justifyContent:'center',padding:16}}>
          <div style={{background:'var(--surface)',border:'1px solid var(--border2)',borderRadius:2,padding:24,width:'100%',maxWidth:420,animation:'slideUp 0.2s ease'}}>
            ...
            <div style={{display:'flex',gap:8,justifyContent:'flex-end',marginTop:14}}>
              <button onClick={()=>setAddModal(false)} style={{padding:'8px 14px',fontFamily:'Barlow Condensed,sans-serif',fontSize:10,fontWeight:700,background:'transparent',color:'var(--text-dim)',border:'1px solid var(--border2)',cursor:'pointer',borderRadius:2}}>CANCEL</button>
              <button onClick={()=>{
                ...
                setAddModal(false); notify('Contact added','success');
