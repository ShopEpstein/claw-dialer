...
export default function ClawDialer() {
  ...
  async function startCall() {
    ...
    try {
      const r = await fetch('/api/twilio/call', {
        ...
        body: JSON.stringify({ to: activeContact.phone, contactId: activeContact.id, contactName: activeContact.name, script: scripts[scriptIdx]?.name })
      });
  ...
  async function setDisposition(outcome) {
    ...
    if (outcome === 'interested') {
      ...
      try {
        await fetch('/api/twilio/sms', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ to: activeContact.phone, body: SMS_FOLLOW_UP(activeContact.name) }) });
      } catch {}
    }
    setCallSeconds(0);
    setNotes('');
  ...
  async function sendSMS() {
    ...
    try {
      const r = await fetch('/api/twilio/sms', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ to: activeContact.phone, body: smsBody }) });
      const data = await r.json();
      if (!r.ok) throw new Error(data.error);
      ...
      {tab === 'dialer' && (
        <div style={{display:'grid',gridTemplateColumns:'320px 1fr 280px',height:'calc(100vh - 90px)',overflow:'hidden'}}>
          ...
          <div style={{borderRight:'1px solid var(--border)',overflow:'hidden',display:'flex',flexDirection:'column'}}>
            ...
            {/* BUG FIX: was gap:6'} — extra quote removed */}
          ...
          <div style={{borderRight:'1px solid var(--border)',display:'flex',flexDirection:'column',overflow:'hidden'}}>
            ...
            <div style={{padding:'18px 20px',borderBottom:'1px solid var(--border)',background:'var(--surface)'}}>
              ...
              <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
                {callState === 'idle' && (
                  <button onClick={startCall} style={{padding:'12px 24px',fontFamily:'Bebas Neue, sans-serif',fontSize:15,letterSpacing:3,background:'var(--green)',color:'var(--bg)',border:'none',cursor:'pointer',borderRadius:2}}>📞 DIAL</button>
                )}
        ...
        <div style={{padding:24,overflowY:'auto',height:'calc(100vh - 90px)'}}>
          ...
          <div>
            ...
            <div style={{display:'flex',gap:10,flexWrap:'wrap'}}>
              ...
              <button onClick={() => { if(confirm('Clear everything?')) { setContacts([]); setCallLog([]); notify('Cleared','warning'); } }} style={{padding:'9px 16px',fontFamily:'Barlow Condensed, sans-serif',fontSize:11,fontWeight:700,background:'var(--red)',color:'white',border:'none',cursor:'pointer',borderRadius:2,marginLeft:'auto'}}>CLEAR ALL</button>
            </div>
          </div>
        </div>
      )}
